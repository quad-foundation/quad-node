...
func TestGenerateBytesAndParseFunctions(t *testing.T) {
	...
	byteSlice, err := GenerateBytesFromParams(sigName, pubKeyLength, privateKeyLength, signatureLength, isValid, isPaused)
	if err != nil {
	...
	parsedSigName, parsedPubKeyLength, parsedPrivateKeyLength, parsedSignatureLength, parsedIsValid, parsedIsPaused, err := GenerateParamsEncryptionSchemesFromBytes(byteSlice)
	if err != nil {
	...
	if parsedIsValid != isValid {
		t.Errorf("IsValid mismatch: got %t, want %t", parsedIsValid, isValid)
	}
